...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        {
            if (find(strupr(s)) && lookup(strlwr(s),0,(dictionary.size))) 
            {
                score += strlen(s);
            }
        }
...
bool lookup(string s, int min, int max)
{
    ...
    if (strcmp(strlwr(dictionary.words[midpoint].letters), s) < 0) 
    {
        // dictionary word comes before s
        min = midpoint + 1;
    }
    else if (strcmp(strlwr(dictionary.words[midpoint].letters), s) > 0) 
    {
        // dictionary word comes after s
        max = midpoint - 1;
    }
    else 
    {
